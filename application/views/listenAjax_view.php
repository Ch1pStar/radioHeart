	...
	$pl_display_name = str_replace("-", " ", $this->edit_pl_model->getPlDisplayNameNew($this->uri->segment(3)));
	include("qrcode.php");
	$qr = new qrcode();
	$m_link = base_url()."index.php/m/listen/$pl_name";
	$qr->link($m_link);
?>
	...
	<div class="pl_title" >
		<div id="title_hide" > 
			<div id="title_display" ><?=$pl_display_name?> 
			<a href="#" class="edit_title" onclick=" edit_pl_title('<?=$pl_name?>'); return false;">Edit</a>
			</div>
		</div>
		<form id="title_edit_form" action="#" style="display:none;">
			<input type="text" id="panda" value="<?=$pl_display_name?>"/>
		...
		<div id="flashContent">
			...
			<div id="playlist_tags">
				
			
			</div>
			<div id="playlist_songs" class="tracks_box">	
			<?=$this->tags_model->getPlTags($pl_name);?>
				<?php if($this->edit_pl_model->getPlAuthor($pl_name)==$username){
					...
					echo "<p><img src=\"".$qr->get_link(100,'L','0')."\"/></p>"
				?>
			</div>
			<?php if ($this->session->userdata('logged_in')&& isset($username)):?>
				<div id="add_comment">
					<form method="post" id="add_comment_form" action="<?=base_url()?>index.php/home/addcomment">
						<textarea name="content" id="content_area" value=" "></textarea><br/> 
